...
loop_row:
        bge     $s0, N_ROWS, reveal_grid__epilogue
        li      $s1, 0                  # int col = 0
        j       loop_col
loop_col:
        bge     $s1, N_COLS, end_col
        la      $t0, grid               # calculate &grid[row][col]
        mul     $t1, $s0, N_COLS
        add     $t2, $t1, $t0
        mul     $t3, $s1, 1
        add     $t4, $t3, $t2
        lb      $t5, ($t4)              # load grid[row][col]
        or      $t5, $t5, IS_RVLD_MASK  # and reveal
        sb      $t5, ($t4)              # store back to &grid[row][col]
...
place_bombs__body:
        ...
        li      $t0, 0                  # int i = 0
        lw      $t1, total_bombs        # load total_bombs
        move    $s0, $a0                # store $a0, $a1, $t0, $t1
        move    $s1, $a1                # into $s0, $s1, $s2, $s3 respectively
        move    $s3, $t1
place_bomb:
        move    $s2, $t0                
        move    $t1, $s3                
        bge     $t0, $t1, place_bombs__epilogue
        move    $a0, $s0                # reload $a0, $a1 and $t1
        move    $a1, $s1
...
mark_cell__body:
        ...
        and     $t6, $t5, IS_RVLD_MASK  # if (grid[row][col] & IS_RVLD_MASK)
        bne     $t6, 0, is_rvld         
        
        lw      $t8, bomb_count
        
        and     $t7, $t5, IS_MRKD_MASK  # if (grid[row][col] & IS_MRKD_MASK)
        bne     $t7, 0, unmark_cell
        
        or      $t5, $t5, IS_MRKD_MASK  # else
        sb      $t5, ($t4)              # mask the cell
        addi    $t8, $t8, -1            # bomb_count--
        sw      $t8, bomb_count         # store bomb_count
        
        j       mark_cell__epilogue
is_rvld:
        lw      $t6, debug_mode         # load debug_mode
        
        beq     $t6, TRUE, mark_cell__epilogue     
                                        # if (debug_mode) {
                                        #     return
                                        # }
        la      $a0, mark_error         # print mark_error
        li      $v0, 4                  # and then return
        syscall
        
        j       mark_cell__epilogue
unmark_cell:                            
        li      $v0, IS_MRKD_MASK       # if the cell has been marked before
        not     $v0, $v0                # unmark it
...
reveal_cell__body:
        ...
        and     $t6, $t5, IS_BOMB_MASK  # $t6 = grid[row][col] & IS_BOMB_MASK
        bne     $t6, 0, game_lose
        

if_marked:
        and     $t6, $t5, IS_MRKD_MASK  # $t6 = grid[row][col] & IS_MRKD_MASK
        bne     $t6, 0, if_marked_body  # if the cell is marked, return
if_revealed:
        and     $t6, $t5, IS_RVLD_MASK  # $t6 = grid[row][col] & IS_RVLD_MASK
        bne     $t6, 0, if_revealed_body
receal_action:
        and     $t6, $t5, VALUE_MASK    # $t6 = grid[row][col] & VALUE_MASK
...
if_marked_body:
        lw      $t7, debug_mode         # load debug_mode
        
        beq     $t7, TRUE, reveal_cell__epilogue
        
        la      $a0, reveal_error       # print error message
        li      $v0, 4
        syscall
...
if_revealed_body:
        ...
        la      $a0, already_revealed   # print already_revealed message
        li      $v0, 4
        syscall
        
        j       reveal_cell__epilogue
game_lose:
        li      $t7, LOSE               # if the cell is a bomb, lose game
        lw      $t8, game_state         # load game_state
        move    $t8, $t7                # update game_state
        sw      $t8, game_state         # store game_state
win_check:
        lw      $t7, cells_left
        bne     $t7, 0, reveal_cell__epilogue
...
clear_surroundings__body:
        ...
        and     $t6, $t5, IS_RVLD_MASK  # $t6 = grid[row][col] & IS_RVLD_MASK
        bne     $t6, 0, clear_surroundings__epilogue
        
        or      $t5, $t5, IS_RVLD_MASK  # $t5 = grid[row][col] | IS_RVLD_MASK
        lw      $t7, cells_left         # cells_left--
        addi    $t7, $t7, -1
        sw      $t7, cells_left
                                        # unmark cell
        li      $t7, IS_MRKD_MASK
        not     $t6, $t7       # $t6 = ~IS_MRKD_MASK
        and     $t5, $t5, $t6           # $t5 = grid[row][col] & $t6
        sb      $t5, ($t4)              # store grid[row][col]
        
        and     $t6, $t5, VALUE_MASK    # if a number cell is reached, return
        bne     $t6, 0, clear_surroundings__epilogue
        
        move    $s0, $a0                # clear grid[row - 1][col]
        move    $s1, $a1
        addi    $a0, $a0, -1    
        jal     clear_surroundings
        
        addi    $a0, $s0, -1            # clear grid[row - 1][col - 1]
        addi    $a1, $s1, -1
        jal     clear_surroundings
        ...
        move    $a0, $s0                # clear grid[row][col - 1]
        addi    $a1, $s1, -1
        jal     clear_surroundings
        ...
        addi    $a0, $s0, 1             # clear grid[row + 1][col]
        move    $a1, $s1
        jal     clear_surroundings
